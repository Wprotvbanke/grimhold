...
| `public/music/main_theme.ogg` | от владельца проекта — главная тема, звучит в лобби; файл `rising_moon2.ogg`, судя по имени — вторая версия [Fantasy — Rising Moon](https://opengameart.org/content/fantasy-rising-moon) (RandomMind, CC0), взят как есть, 1.8 МБ | уточнить у владельца |
...
| `public/models/town_hall.glb` | от владельца проекта — ратуша, выгрузка FBX; 2.10 → 0.54 МБ, рост 11 м, текстура 2048 (на 512 владелец забраковал), `scripts/prepare-town.ts` | CC, со слов владельца (вид не указан) |
| `public/models/village_house9.glb` | от владельца проекта — сруб с крыльцом, выгрузка FBX; 3.68 → 0.04 МБ, рост 9 м, `scripts/prepare-town.ts` | CC, со слов владельца (вид не указан) |
| `public/models/statue_angel.glb` | от владельца проекта — каменный ангел, памятник на казне; 3.31 → 0.26 МБ, рост сведён к `BANK_STATUE.height` (восемь метров), `scripts/prepare-town.ts` | CC, со слов владельца (вид не указан) |
...
| `public/ui/face/{full,fight,hit,low,trade,trade_end}.webp` | от владельца проекта — картинки настроения лица в панели (`Human_Face_Radar`, 343×342 → 256, по 10–12 КБ, `scripts/prepare-panel.ts`); какая когда — docs/panel.md | со слов владельца (вид не указан) |
| `public/ui/panel.webp` | от владельца проекта — каменная панель с горгульями внизу экрана (`bars/bar2.png`, 1600×335, 293 → 58 КБ, `scripts/prepare-panel.ts`); раскладка по её вырезам — docs/panel.md | со слов владельца (вид не указан) |
| `public/models/rain_cloud.glb` | от владельца проекта, выгрузка Sketchfab «Clouds rain» — туча с дождём и молниями, один клип; заготовка погодной магии (docs/mage_skills.md), в мире пока не стоит; 0.71 → 0.25 МБ, рост сведён к `RAIN_CLOUD.height`, `scripts/prepare-town.ts rain_cloud` | CC, со слов владельца (вид не указан) |
...
| `public/models/bow.glb` | от владельца проекта, `Bow/Bow.fbx`; собран `scripts/prepare-weapon.ts`, 438 треугольников, 12 КБ. Клип `rig|Bow_Shot` из `Bow/BowAnim.fbx` перенесён в руки `prepare-sword.ts` | уточнить |
...
Перенос через грудь с поправкой на позу покоя мы писали (`prepare-bow.ts`)
и **убрали 16 сентября вместе с анимацией лука**: движение выходило
приблизительным, и владелец забраковал его целиком. Что осталось от той
работы в виде правил — в [hands.md](hands.md), «Тупики».

**Правильный путь — просить анимации на нашем риге.** Так сделаны
`Sword_Idle` и `Sword_Slash` от владельца: имена костей совпадают номер
в номер, переносить нечего, приближения нет (`prepare-sword.ts`).
...
npx tsx scripts/prepare-town.ts       # здания, таверна, дома, повозка, казна, памятник, стены (главный сборщик города)
...
**Сжатие текстуры решает глаз, а не правило.** Общая мерка для города —
512, и восьми домам пака её хватает. Ратуше не хватило: у неё исходник
4096, весь рисунок в мелкой резьбе и переплётах, и на 512 она расползлась в кашу —
владелец забраковал с первого взгляда. У неё теперь 2048 (`texture: 2048`),
и файл вырос с 0.12 до 0.54 МБ. Здание одно на город — это тот случай, когда
полмегабайта стоят того; для повторяющихся домов так делать не надо.

**Здания принимаются и в FBX.** `prepare-town.ts` сам видит расширение
и переводит выгрузку в glTF в памяти, без промежуточного файла на диске:
такой файл однажды устареет, и соберётся не тот дом. Две ловушки, на которые
мы уже наступили:

- **Текстура идёт отдельно от геометрии.** `GLTFExporter` требует настоящее
  `<img>` с пикселями, а в node его нет — экспорт падает на «No valid image
  data». Поэтому материалы пересобираются без картинок, а сама картинка
  вынимается из FBX по её собственной подписи (PNG или JPEG) и вкладывается
  уже в glTF — тот же путь, что у оружия.
- **Развёртка переворачивается по вертикали**: у FBX начало внизу, у glTF
  — вверху. Без этого рисунок на стенах уезжает.
