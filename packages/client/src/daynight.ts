  ...
  const api: DayNight = {
    ...
    update(time, camera) {
      ...
      if (api.underground) {
         ...
         *
         * Видно на пять с половиной метров, дальше — глухо. Это уже не атмосфера,
         * а механика: сундук находят, подойдя вплотную, а нежить выходит
         * из мглы на расстоянии удара.
         *
         * Постоянного огня внизу больше нет — его убрали вместе с настенными
         * факелами. Сквозь мглу рисуется только **знак выхода** (`fog: false`),
         * и он единственный маяк; всё остальное освещает игрок сам.
         *
         * Поэтому остаток общего света трогать нельзя: он и есть то, что
         * отличает тёмный зал от чёрного экрана, в котором ищут не выход,
