...
export function createDayNight(scene: THREE.Scene, sky: Sky): DayNight {
  ...
  const api: DayNight = {
    ...
    update(time, camera) {
      ...
      if (api.underground) {
        /**
         * Теперь это темнота, а не полумрак.
         *
         * Первая попытка была честно тёмной — и зал приняли за несуществующий:
         * ни стен, ни пола, ни текстур. Тогда свет подняли, чтобы место
         * читалось. Держалось это ровно до факела: **теперь есть чем светить
         * самому**, и общий свет снова вдвое ниже.
         *
         * Ниже опускать нельзя без предупреждения: остаток — это не «чтобы
         * было видно», а чтобы стены угадывались на расстоянии вытянутой руки
         * и без факела было понятно, что ты в зале, а не в пустоте.
         */
         ...
         * Подземная мгла.
         *
         * Много гуще городского тумана: под землёй воздух стоячий, и даль зала
         * обязана тонуть — иначе подземелье читается как большая комната,
         * а не как место, где что-то может ждать в трёх шагах.
         *
         * Видно на четыре метра, дальше — глухо. Это уже не атмосфера,
         * а механика: сундук находят, подойдя вплотную, а нежить выходит
         * из мглы на расстоянии удара.
         *
         * Держится это целиком на том, что **сквозь туман рисуется огонь**:
         * ореолы факелов (`fog: false` в lights.ts) и знак выхода видны, когда
         * камень уже растворился. Дорога читается по огням, а не по стенам.
         *
         * Без них такая мгла — чёрный экран, в котором ищут не выход,
         * а поломку. На этом уже обжигались: первую честную темноту приняли
         * за несуществующий зал.
         */
