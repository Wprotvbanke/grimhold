  ...
  const api: DayNight = {
    ...
    update(time, camera) {
      ...
      if (api.underground) {
         ...
         *
         * Дальняя стена в такой мгле не видна, и это намеренно. Ориентиром
         * служит не она, а огонь: ореолы факелов нарисованы **сквозь** туман
         * (`fog: false` в lights.ts) и потому остаются видны, когда камень уже
         * растворился. Дорога к выходу читается по огням — так и задумано.
         *
         * Без этих ореолов такую мглу ставить нельзя: получится чёрный экран,
         * в котором ищут не выход, а поломку. На этом уже обжигались.
         */
        fog.color.setHex(0x120c0f);
        fog.near = 3;
        fog.far = 42;
