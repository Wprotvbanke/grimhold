  ...
  const api: DayNight = {
    ...
    update(time, camera) {
      ...
      if (api.underground) {
         ...
         * Подземная мгла.
         *
         * Ближе городского тумана и гуще: под землёй воздух стоячий, и даль
         * зала обязана тонуть — иначе подземелье читается как большая комната,
         * а не как место, где что-то может ждать в трёх шагах.
         *
         * Но не настолько, чтобы потерять стены: зал шестьдесят на шестьдесят,
         * и дальняя стена должна угадываться. Отсюда `far` в семьдесят —
         * противоположный конец различим, но без подробностей.
         */
        fog.color.setHex(0x150f12);
        fog.near = 6;
        fog.far = 70;
