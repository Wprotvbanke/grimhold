...
/**
 * Затемнение по краям кадра, пока идёт медитация.
 *
 * Слой чисто рисовальный: сервер о нём не знает, клиент включает его по
 * `meditating` из снапшота. Помним прошлое состояние, чтобы трогать DOM
 * на переходе, а не каждый кадр.
 */
const vignette = document.getElementById('meditation') as HTMLDivElement;
...
renderer.setAnimationLoop((frameTime: number) => {
  ...
  if (game) {
    ...
    /**
     * Виньетка медитации — картинка и ничего больше.
     *
     * Сервер о ней не знает: он шлёт только «медитирую», а затемнение краёв
     * клиент выводит сам, как выводит свет от факела. Класс переключаем
     * лишь на переходе — трогать DOM двадцать раз в секунду ради ничего
     * не изменившегося незачем, а заодно не сбивается дыхание виньетки.
     */
    const calm = self?.meditating === true;
    if (calm !== meditating) {
      meditating = calm;
      vignette.classList.toggle('on', calm);
    }
