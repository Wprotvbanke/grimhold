 ...
 *
 * Слышно в восьми метрах от люка: мимо проходят и слышат, но площадь
 * не звучит подземельем. Громкость растёт к люку плавно — граница радиуса
 * не должна щёлкать.
 */
const GATE_HEARING = 8;
/**
 * Половину громкости подземелья владелец едва слышал, три четверти — слышал,
 * но просил ещё на четверть. У самого люка теперь почти как внизу: люк
 * и должен тянуть к себе, а дальше пяти метров его всё так же не слышно.
 */
const GATE_LOUDNESS = 0.95;
