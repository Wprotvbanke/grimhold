...
export class Controls {
   ...
   * блок, Ctrl — рывок в сторону движения, цифры — панель, Shift — бег.
   *
  ...
  private wireCombat(): void {
    ...
    window.addEventListener('keydown', (event) => {
      ...
      if (event.code === 'ControlLeft' || event.code === 'ControlRight') {
         ...
         * Рывок на Ctrl.
         *
         * `repeat` отсекается намеренно: Ctrl часто держат, а не нажимают,
         * и без этого зажатая клавиша слала бы намерение по десять раз
         * в секунду — сервер отбивал бы их откатом, но руки дёргались бы.
         *
