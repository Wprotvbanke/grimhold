...
/**
 * Туман из люка: модель в 7 м шириной ужата до 5.5 — шире рамы, чтобы
 * переливался через край на мостовую. Слои у модели лежат на 0.2…0.95 м ниже
 * её нуля: подъём ставит верхний слой невысоко над рамой (верх рамы — 0.19 м).
 */
const HATCH_FOG = {
  // Первый был 0.4; владелец просил сперва на 30% больше и выше, потом ещё
  // в полтора раза больше и пониже. Слои при таком размере уходят до четверти
  // метра под мостовую — нижние прячет чернота проёма, верхний держится
  // в 35 см над землёй, чуть выше рамы.
  scale: 0.78,
  lift: 0.5,
  opacity: 0.6,
  /**
