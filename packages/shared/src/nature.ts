 ...
 *
 * Обычного дерева, сухостоя, валунов и крупных кустов тут нет намеренно: эти
 * модели отданы ресурсным нодам ([nodes.ts](nodes.ts)). Иначе игрок не отличит
 * дерево, которое рубится, от дерева, которое просто стоит, — и будет
 * колотить топором по декорации. Сухостой ушёл последним: с него ломают
 * ветки, и в мире не должно остаться ни одного, который нельзя обломать.
 */
