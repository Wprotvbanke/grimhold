...
/** Опыт за удачно принятый щитом удар. */
export const EXPERIENCE_PER_DEFENCE = 4;

/**
 * Опыт за **ответ после ухода**: ушёл рывком от удара и тут же достал врага.
 *
 * Уклонение учится только так — не самим уходом. Рывок сам по себе делают
 * и просто так: прыгнул в сторону от крысы, которая даже не била, и навык
 * рос. Уйти от настоящего удара и наказать за него — это уже мастерство,
 * а не привычка жать кнопку.
 *
 * Платится чуть щедрее блока: условие вдвое строже, и без прибавки навык рос
 * бы втрое медленнее остальных.
 */
export const EXPERIENCE_PER_RIPOSTE = 6;

/**
 * Сколько секунд после ухода засчитывается ответ.
 *
 * Три — это успеть развернуться и достать. Больше значило бы «когда-нибудь
 * потом», и связь между уходом и ударом перестала бы читаться.
 */
export const RIPOSTE_SECONDS = 3;
/**
