 ...
 *
 * **Растёт вместе с дальнобойным оружием, а не отдельно от него.** Стрелять
 * дальше этого радиуса невозможно по построению: цели, которую не прислали,
 * на экране нет. Было сорок пять при луке в тридцать пять; лук вырос до сорока
 * пяти — вырос и радиус, с прежним запасом.
 *
 * Цена известна и измерена: это площадь, то есть число сущностей в снапшоте
 * (см. `npm run load`). Поэтому число не поднимают «на всякий случай» —
 * только следом за дальностью, которой оно служит.
 */
export const AOI_RADIUS = 58;
...
export interface EntitySnapshot {
  ...
  ally?: boolean;
  /**
   * Работает у ноды или вскрывает сундук.
   *
   * Нужен клиенту ради звука. Замысел обещал, что вскрывающий сундук «уязвим
   * и слышен», а о чужой работе клиенту больше не сообщает ничто: полоса
   * работы уходит только самому работнику. Вывести это из снапшота нечем —
   * значит надо сказать.
   */
  work?: 'node' | 'chest';
}
