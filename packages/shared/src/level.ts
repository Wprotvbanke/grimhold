...
export const BANK_STATUE = {
  /**
   * Рост памятника в метрах.
   *
   * Отсюда его берёт сборка модели (`prepare-town.ts`): число одно,
   * иначе памятник в игре и число в коде разошлись бы при первой же
   * правке размера. Восемь метров — вдвое крупнее первого роста,
   * так решил владелец, глядя на площадь.
   */
  height: 8,
} as const;

/**
 * Туча с дождём над площадью — **проба**, один экземпляр.
 *
 * Владелец принёс модель и хочет посмотреть, как она выглядит в игре и как
 * читаются капли. Висит над серединой площади, сервер о ней не знает —
 * это картинка. Рост общей модели (облака и дождь под ними) и высота её
 * нижнего края над мостовой в метрах; рост берёт сборка (`prepare-town.ts`),
 * как у памятника, поэтому число живёт здесь.
 */
export const RAIN_CLOUD = {
  x: 0,
  z: 0,
  /** Рост модели целиком: от нижних капель до макушки облаков. */
  height: 24,
  /** Нижний край (где капли кончаются) над землёй. */
  lift: 2,
} as const;
