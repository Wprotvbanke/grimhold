...
/**
 * Туча с дождём — заготовка **погодной магии мага** (docs/mage_skills.md).
 *
 * Владелец повесил её над площадью на пробу, посмотрел и снял: в мире
 * её нет, модель `rain_cloud.glb` собрана и ждёт свитка. Рост берёт
 * сборка (`prepare-town.ts`), как у памятника, поэтому число живёт здесь.
 */
