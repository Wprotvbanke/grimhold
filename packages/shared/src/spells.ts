...
export const SPELLS: Record<SpellId, SpellProfile> = {
  ...
  meditation: {
    ...
    description:
      ...
      'повторное нажатие, и тогда семь секунд не сесть снова. Мана только себе.',
    category: 'support',
    skill: 'restoration',
    shape: 'channel',
    manaCost: 0,
    castTime: 0.5,
    /**
     * Откат отсчитывается **от конца медитации**, а не от начала.
     *
     * Семь секунд после того, как встал, — это и есть цена места: сесть
     * в проходе и вскакивать на каждый шорох не выйдет, придётся выбирать,
     * где садиться. Иначе медитацию щёлкали бы туда-сюда без потерь.
     */
    cooldown: 7,
    power: 0,
    range: 0,
