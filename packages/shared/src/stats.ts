...
/**
 * Мана сама не восстанавливается. Совсем.
 *
 * Она устроена как здоровье, а не как стамина: кончилась — значит кончилась,
 * и вернуть её можно только зельем или медитацией. Пока она натекала сама
 * по три с половиной в секунду, заклинания были бесплатными — достаточно
 * было постоять полминуты, и весь расчёт «чем платить за силу» пропадал.
 *
 * Функция оставлена нарочно: место, где мана прибывала бы сама, должно быть
 * одно и на виду. Появится кольцо или посох с отдачей маны — прибавка ляжет
 * сюда, а не размажется по тику.
 */
export function manaRegen(_attributes: Attributes): number {
  return 0;
}
