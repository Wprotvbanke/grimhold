...
export function handleAdmin(
   ...
   * Ранить себя.
   *
   * Отладочный ключ: полосу жизни, свечение лечения и смерть иначе не
   * проверить — сам себя игрок ударить не может, а идти за этим к волку
   * долго и ненадёжно. Бьёт вполовину нынешнего здоровья и никогда
   * не убивает: смерть проверяется своим путём.
   */
  if (message.do === 'hurt') {
    const combat = actor.combat;
    if (!combat.alive) return [{ type: 'itemError', reason: 'Мёртвого не ранить' }];
    combat.vitals.health = Math.max(1, Math.floor(combat.vitals.health / 2));
    return [];
  }
