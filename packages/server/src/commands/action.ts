...
export function beginCast(
  ...
  /**
   * Садиться с полным запасом незачем — и вредно.
   *
   * Медитация кончается сама, когда мана полна, поэтому нажатие на полном
   * запасе включало её и гасило в тот же тик: игрок видел, что «ничего
   * не произошло» — ни маны, ни виньетки, — и справедливо считал это
   * поломкой. Отказ вслух честнее молчаливого мигания.
   */
  if (SPELLS[spellId].shape === 'channel' && combat.vitals.mana >= actor.maxima.mana) {
    return 'Мана и так полна';
  }
